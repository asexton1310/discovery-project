...
def p1MetricsLoop(framesfolder_path, vidname, csv_out):
    ...
    for frame in frame_list:
        freeze_frame[1] = freeze_frame[1] + 1
        full_path = framesfolder_path + frame
        # some metrics require frame to already be read with opencv
        cv_frame = cv2.imread(full_path)
        # some metrics require an array of frame data. So build it
        frame_data.append(cv_frame)
        if abs(np.mean(frame_prev) - np.mean(cv_frame)) > 0.003:
            ...
            freeze_frame[0] = freeze_frame[0] + 1
            frame_data.append(cv_frame)
    ...
    csv1.append(vidname)
    # FREEZE FRAME
    csv1.append(freeze_frame[0]/freeze_frame[1])
    # BLOCKINESS
...
def p2MetricsLoop(framesfolder_path, vidname, csv_out):
    ...
    print("Time Elapsed for P2: ", time.perf_counter() - start_time)
...
def p3MetricsLoop(framesfolder_path, vidname, csv_out):
    ...
    for frame in frame_list:
        ...
        cv_frame = cv2.imread(full_path)
        frame_data.append(cv_frame)
        if abs(np.mean(frame_prev) - np.mean(cv_frame)) > 0.003:  # check for freeze-frame
            frame_prev = cv_frame
    ...
    print("Time Elapsed for P3: ", time.perf_counter() - start_time)
...
if __name__ == "__main__":
    ...
    csv_label = ['video_name', 'freeze_frame', 'block_avg', 'block_max', 'block_min', "blur_avg", "blur_max", "blur_min", 'avg_color1', 'max_color1',
    ...
    for frame_folder in os.listdir(output_path):
        ...
        name = "video"
        with open(f'quality-metrics/{name}-{video_num}.csv', 'a', newline='') as csvfile:
            metric_writer = csv.writer(csvfile, delimiter=',')
