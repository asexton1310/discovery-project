...
class NewFrameEventHandler(PatternMatchingEventHandler):
    ...
    def on_created(self, event):
        ...
        shutil.move(
            target_file, f"finishedSamples/{os.path.basename(target_file)}")
...
def buildDeploymentCSV(path):
    ...
    # Process p2 (brisque, noise, ltp)
    p3 = mp.Process(
        target=p3MetricsLoop,
        args=(path + "/", vidname + ".mp4", frame_list, csv_in3),
    )
    # Process p4 (bitrate, framerate, resolution)
    p4 = mp.Process(
    ...
    csv_out6 = csv_out6.recv()  # return list of csv values for p5 metrics(SI, TI)
    p1.join()
    p2.join()
    p3.join()
    p4.join()
    ...
    # Acheive total CSV list with appropriate outputs
    csv_out = (
        csv_out1 + csv_out2 + csv_out3 + csv_out4 + csv_out5 + csv_out6
    )
