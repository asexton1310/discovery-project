...
def buildDeploymentCSV(path):
    ...
    p5_siti.terminate()
    # Acheive total CSV list with appropriate outputs
    csv_out = csv_out1 + csv_out2 + csv_out4 + csv_out5
...
def p4_AGH_tool(path, csv_out):
    ...
    for frames in os.listdir(path):
        absolute_path = os.path.abspath(frames)
        # print(f"frame name: {frames}")
        agh_list = AGH_getResutls(absolute_path)
        # print("AGH_LIST is below:\n", agh_list)
        flicker.append(agh_list[0])
        block.append(agh_list[1])
        letterBox.append(agh_list[2])
        pillarBox.append(agh_list[3])
        blockloss.append(agh_list[4])
        blur.append(agh_list[5])
        blackout.append(agh_list[6])
        freezing.append(agh_list[7])
        exposure.append(agh_list[8])
        contrast.append(agh_list[9])
        interlace.append(agh_list[10])
        noise.append(agh_list[11])

    csv4.append(np.average(flicker))
    csv4.append(np.average(block))
    csv4.append(np.average(letterBox))
    csv4.append(np.average(pillarBox))
    csv4.append(np.average(blockloss))
    csv4.append(np.average(blur))
    csv4.append(np.average(blackout))
    csv4.append(np.average(freezing))
    csv4.append(np.average(exposure))
    csv4.append(np.average(contrast))
    csv4.append(np.average(interlace))
    csv4.append(np.average(noise))
...
def p5_si_ti(path, csv_out):
    ...
    for x in range(len(frame_list)):
        frame, ext = frame_list[x].split(".")
        frame, no = frame.split("-")
        num.append(no)
    num = [int(i) for i in num]
    num.sort()
    for x in range(len(frame_list)):
        frame_list[x] = f"{path}/frame-{num[x]}.png"
    siti_metrics = sitiExtraction.calculateSiTi(frame_list)
    # get average si
    si = np.average(siti_metrics[0])
    csv5.append(si)
    # get average ti
    siti_metrics[1].pop(0) # remove first frame's ti since this is an invalid score
    ti = np.average(siti_metrics[1])
    csv5.append(ti)
