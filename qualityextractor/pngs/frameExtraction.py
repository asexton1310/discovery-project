...
def saveFrames(frames, step, filename, output_dir):
    ...
    for i, frame in enumerate(frames):
       ...
       # print(f"imwrite(...): {output_dir}/{filename}-f{frame_pos:0{digits}}.png")
        cv2.imwrite(
            f"{output_dir}/{filename}-f{frame_pos:0{digits}}.png", frame)
...
def extractFrames(in_filepath, sample_rate):
    ...
    # cap sample_rate if it is higher than video fps
    sample_rate = min(video_fps, sample_rate)
    ...
    while cap.isOpened():
        ...
        if ret:
            if (frame_pos % step) < 1:
                # save the frame
                print(
                    f"imwrite(...): {output_dir}/{ts}-f{frame_pos:0{frame_digits}}.png")
                cv2.imwrite(
                    f"{output_dir}/{ts}-f{frame_pos:0{frame_digits}}.png", frame)
            cv2.imshow("op", frame)  # display it for funzies
        if cv2.waitKey(1) == 27:
            #Esc to quit
            break
...
def sampleStreamCMDline(stream_address, sample_rate, output_path):
    ...
    os.system(
        f"ffmpeg -c:v h264 -i {stream_address} -vf fps={sample_rate} {output_path}sample-%d.png")
...
if __name__ == "__main__":
    ...
    sampleStream('./live-test/A071-03.mp4', './raw-frames/')
