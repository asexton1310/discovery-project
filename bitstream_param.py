...
def encodeH264stream(in_filename, output_path):
    ...
    os.system(f"ffmpeg -i {in_filename} -c:v libx264 -y -f h264 {output_filename}")
...
def extractH264streamParams(in_filename, output_path):
    ...
    os.system(f"ffprobe -show_streams -i {in_filename} -of default=nw=1 > {output_filename}") # for human readable results
